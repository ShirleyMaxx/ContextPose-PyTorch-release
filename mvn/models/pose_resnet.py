...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, num_joints, num_final_layer_channel,
        ...
        self.num_final_layer_channel = num_final_layer_channel
        ...
        self.final_layer = nn.Conv2d(
            ...
            out_channels=self.num_final_layer_channel,
...
def get_pose_net(config):
    ...
    model = PoseResNet(
        block_class, layers, config.num_joints, config.num_final_layer_channel,
        num_input_channels=3,
        deconv_with_bias=False,
        num_deconv_layers=3,
        num_deconv_filters=(256, 256, 256),
    ...
    if config.init_weights:
        if config.fix_weights:
            print("Loading weights for Backbone2D from: {}".format(config.checkpoint))
            ...
            print("Successfully loaded (fixed) weights for Backbone2D model")
        else:
            print("Loading pretrained weights from: {}".format(config.checkpoint))
            model_state_dict = model.state_dict()
            pretrained_state_dict = torch.load(config.checkpoint, map_location='cpu')
